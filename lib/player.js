...
module.exports = function (spotify, queue, playlist) {
  ...
  var firebaseStatus = firebase.ref.database().ref("player/playing");
  var firebaseNext = firebase.ref.database().ref("player/next");
  var firebaseCurrent = firebase.ref.database().ref("player/current_track");
  var firebaseVolume = firebase.ref.database().ref("volume");
  ...
  function setCurrentTrack(track){
    if (track) {
      currentTrack = track;
      firebaseCurrent.set({uri: track.uri, started_at: firebase.firebase.database.ServerValue.TIMESTAMP});
